...
async def test_list_geospatial_layers_wfs():
    mock_caps = {
        ...
        "layers": [
            {
                "name": "msp:casernes_incendie",
                "title": "Casernes d'incendie",
                "abstract": "Localisation des casernes",
            },
            {"name": "msp:postes_police", "title": "Postes de police", "abstract": ""},
            {
                "name": "mern:ecoles",
                "title": "Écoles du Québec",
                "abstract": "Établissements scolaires",
            },
        ],
    }
    with patch(
        "tools.list_geospatial_layers.geo_api.get_wfs_capabilities",
...
async def test_list_geospatial_layers_wms():
    mock_caps = {
        "total": 1,
        "layers": [{"name": "carte_base", "title": "Carte de base", "abstract": ""}],
    }
    with patch(
        "tools.list_geospatial_layers.geo_api.get_wms_capabilities",
        new_callable=AsyncMock,
        return_value=mock_caps,
    ):
        from tools.list_geospatial_layers import list_geospatial_layers
...
async def test_get_geospatial_features():
    ...
    with patch(
        "tools.get_geospatial_features.geo_api.get_features",
        new_callable=AsyncMock,
        ...
        result = json.loads(
            await get_geospatial_features(layer_name="msp:casernes_incendie"),
        )
...
async def test_get_geospatial_features_error():
    with patch(
        "tools.get_geospatial_features.geo_api.get_features",
        new_callable=AsyncMock,
        side_effect=RuntimeError("couche introuvable"),
    ):
        from tools.get_geospatial_features import get_geospatial_features

        result = json.loads(
            await get_geospatial_features(layer_name="inexistant"),
        )
        assert "error" in result
...
async def test_describe_geospatial_layer():
    ...
    with patch(
        "tools.describe_geospatial_layer.geo_api.describe_feature_type",
        new_callable=AsyncMock,
        ...
        result = json.loads(
            await describe_geospatial_layer(layer_name="msp:casernes_incendie"),
        )
...
async def test_describe_geospatial_layer_error():
    with patch(
        "tools.describe_geospatial_layer.geo_api.describe_feature_type",
        new_callable=AsyncMock,
        side_effect=RuntimeError("erreur"),
    ):
        from tools.describe_geospatial_layer import describe_geospatial_layer

        result = json.loads(
            await describe_geospatial_layer(layer_name="inexistant"),
        )
        assert "error" in result
...
async def test_get_map_url():
    ...
    result = json.loads(
        await get_map_url(
            layers="msp:casernes_incendie",
            bbox="-73.7,45.4,-73.4,45.6",
        ),
    )
...
async def test_get_map_url_custom_size():
    ...
    result = json.loads(
        await get_map_url(layers="test", bbox="0,0,1,1", width=1024, height=768),
    )
