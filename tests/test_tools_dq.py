...
async def test_search_datasets_returns_results():
    ...
    with patch("tools.search_datasets.ckan_api.package_search", new_callable=AsyncMock, return_value=mock_result):
...
async def test_search_datasets_with_filters():
    ...
    with patch("tools.search_datasets.ckan_api.package_search", new_callable=AsyncMock, return_value=mock_result) as mock:
        from tools.search_datasets import search_datasets

        await search_datasets(query="transport", organization="ville-de-montreal", tags="bus,métro")
        call_kwargs = mock.call_args
        assert "organization:ville-de-montreal" in call_kwargs.kwargs.get("fq", call_kwargs[1].get("fq", ""))
...
async def test_search_datasets_error_returns_json():
    with patch("tools.search_datasets.ckan_api.package_search", new_callable=AsyncMock, side_effect=RuntimeError("timeout")):
        from tools.search_datasets import search_datasets
...
async def test_get_dataset_info_returns_metadata():
    ...
    with patch("tools.get_dataset_info.ckan_api.package_show", new_callable=AsyncMock, return_value=mock_pkg):
...
async def test_get_dataset_info_error():
    with patch("tools.get_dataset_info.ckan_api.package_show", new_callable=AsyncMock, side_effect=RuntimeError("Not found")):
        from tools.get_dataset_info import get_dataset_info
...
async def test_list_dataset_resources():
    mock_pkg = {
        "title": "Actes criminels",
        "resources": [
            {"id": "r1", "name": "2024.csv", "format": "CSV", "size": 500, "datastore_active": True, "url": "https://x.com/1", "last_modified": "2024-01-01"},
            {"id": "r2", "name": "2023.csv", "format": "CSV", "size": 400, "datastore_active": False, "url": "https://x.com/2", "last_modified": "2023-01-01"},
        ],
    }
    with patch("tools.list_dataset_resources.ckan_api.package_show", new_callable=AsyncMock, return_value=mock_pkg):
...
async def test_get_resource_info():
    ...
    with patch("tools.get_resource_info.ckan_api.resource_show", new_callable=AsyncMock, return_value=mock_res):
...
async def test_query_resource_data():
    ...
    with patch("tools.query_resource_data.ckan_api.datastore_search", new_callable=AsyncMock, return_value=mock_result):
...
async def test_query_resource_data_invalid_filters():
    ...
    result = json.loads(await query_resource_data(resource_id="r1", filters="pas du json"))
    assert "error" in result
...
async def test_query_resource_sql():
    ...
    with patch("tools.query_resource_sql.ckan_api.datastore_search_sql", new_callable=AsyncMock, return_value=mock_result):
        from tools.query_resource_sql import query_resource_sql

        result = json.loads(await query_resource_sql(sql='SELECT ville, COUNT(*) as total FROM "r1" GROUP BY ville'))
...
async def test_list_organizations():
    mock_orgs = [
        {"name": "ville-de-montreal", "display_name": "Ville de Montréal", "package_count": 200, "description": ""},
        {"name": "msp", "display_name": "Min. Sécurité publique", "package_count": 50, "description": ""},
    ]
    with patch("tools.list_organizations.ckan_api.organization_list", new_callable=AsyncMock, return_value=mock_orgs):
...
async def test_get_organization_info():
    ...
    with patch("tools.get_organization_info.ckan_api.organization_show", new_callable=AsyncMock, return_value=mock_org):
...
async def test_get_catalog_stats():
    ...
    with (
        patch("tools.get_catalog_stats.ckan_api.package_search", new_callable=AsyncMock, return_value=mock_search),
        patch("tools.get_catalog_stats.ckan_api.organization_list", new_callable=AsyncMock, return_value=mock_orgs),
    ):
